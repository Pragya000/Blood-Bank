...
export default function HospitalDetails() {
    ...
    return (
        <div className="text-lg pb-10">
            <p className='text-gray-700 font-medium mt-4 mb-6'>View Your Details:</p>
            ...
            {user?.additionalFields?.hospitalImages && user?.additionalFields?.hospitalImages.length > 0 ? (
                ...
                <div className="flex flex-wrap gap-4 mt-4">
                    {user?.additionalFields?.hospitalImages?.map((image, index) => (
                        <img key={index} src={image} alt={`Hospital Image ${index}`} className="w-full h-auto max-w-[300px]" />
                    ))}
                    </div>
                </div>
            ) : null}

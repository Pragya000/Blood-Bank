            ...
            <div className="min-h-[calc(100vh-84px)] w-11/12 max-w-[1200px] mx-auto grid place-content-center relative">
                {isLoading ? (
                    ...
                    <span>Back</span>
                        ...
                        <HospitalCard user={data?.data?.hospital} handleRequest={handleRequest} />
                        </div>
                        <div className="flex flex-wrap gap-4">
                            {data?.data?.hospital?.additionalFields?.registrationCertificate ? (
                                <img
                                    src={data?.data?.hospital?.additionalFields?.registrationCertificate}
                                    alt="Registration Certificate"
                                    className="max-w-[250px] max-h-[250px] aspect-square object-cover"
                                />
                            ) : null}
                            {data?.data?.hospital?.additionalFields?.hospitalImages?.map((img, idx) => (
                                <img
                                    key={idx}
                                    src={img}
